      ...
      case Defn.Class(_, className, _, Ctor.Primary(_, _, paramss), _) =>
        paramss.toList match {
          case params :: _ =>
            val implicitReaders =
              collectParamTypesAndNames(params).toList.map {
                case (paramType, paramName) =>
                  val readerName = Term.Name(paramName.syntax + "Reader")

                  q"""
                  implicit def $readerName: cats.data.Reader[$className, $paramType] =
            ...
            def readerIdentity =
              q"""
                implicit def ${Term.Name(className.syntax.uncapitalize+"Reader")}: cats.data.Reader[$className, $className] = {
